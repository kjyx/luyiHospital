        ...
        <div class="top-info-wrap clearfix">
          <div class="left fl wow fadeInLeft">
            <div class="l-s-pic-wrap">
              <div class="swiper-container swiper-index-new" ref="newsSwiper">
                <div class="swiper-wrapper" ref="swiper">
                  <div class="swiper-slide" v-for="(item,index) in swiperNewsList" :key="item.id" @click="$router.push({path:`/newsInfo/${item.id}`})">
                    <img :src="item.pic" alt="" style="width: 100%;height: 100%;" />
                  </div>
                </div>
                <div class="tabs-list">
                  <ul class="clearfix" id="js-tabs">
                    <li class="item alt-1 " :class="{'active': index === current}" v-for="(item3,index) in swiperNewsList" :key="item3.id" @click="switchChange(index)" ref="li">
                      <div class="box">
            ...
            <div class="l-info-wrap" id="js-info">
              <a class="block info-cont " v-for="(item2,index) in swiperNewsList" :key="item2.id" :class="{'active': current === index}" @click="$router.push({path:`/newsInfo/${item.id}`})">
            ...
            <div class="r-box-wrap">
              ...
              <div class="r-content" id="js-dt-content">
                <ul class="r-list active">
                  <li class="item" v-for="item in DynamicAndNotice" :key="item.id" @click="$router.push({path:`/newsInfo/${item.id}`})">
                    <a  class="block">
...
export default {
  ...
  methods:{
    // 获取新闻信息
     getNewsList(id,pagenum = 1){
       this.pageParmise.pageNum = pagenum
...
<style scoped>
.dynamic-wrap .top { background: url("~~@/assets/images/newsbeijing.jpg") no-repeat center center; -o-background-size: cover; background-size: cover; padding-bottom: 30px; }
